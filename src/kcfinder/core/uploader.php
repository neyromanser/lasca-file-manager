...
namespace Neyromanser\LascaFileManager\Kcfinder;

use Neyromanser\LascaFileManager\Kcfinder\helper_text as text;
use Neyromanser\LascaFileManager\Kcfinder\img;
use Neyromanser\LascaFileManager\Kcfinder\mime;
use Config;
use Session;
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        $config = Config('lasca-file-manager');
        $_CONFIG = $config;
        view()->share('lascaFileConfig', $config);
        ...
        */
        ...
        if (isset($_CONFIG[$sessVar])) {
            ...
            //echo $sessVar;
            //session([$sessVar => 'asdf']);

           // if(!session($sessVar)) echo 'no sess';
            if(!session($sessVar)) session([$sessVar => []]);
            //if (!isset($_SESSION[$sessVar])) $_SESSION[$sessVar] = array();

            //$sessVar = &$_SESSION[$sessVar];
            $sessVar = session($sessVar);
            ...
            if($this->session) session([$sessVar => $this->session]);
            //$this->session = &$sessVar['self'];
        ...
        foreach ($this->langInputNames as $key)
            if (isset($_GET[$key]) &&
                ...
                file_exists(__DIR__."/../lang/" . strtolower($_GET[$key]) . ".php")
            ) {
                $this->lang = $_GET[$key];
                break;
            }
        ...
        view()->share('fileManagerLang', $this->lang);
        view()->share('fileManagerSession', $this->session);
        view()->share('fileManagerOpener', $this->opener);
        view()->share('fileManagerType', $this->type);
    ...
    protected function localize($langCode) {
        ...
        $this->labels = $lang;
        view()->share('localizedLabels', $this->labels);
